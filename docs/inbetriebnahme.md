...
```bash
python3 scripts/nr.py check <inst>                              # sauber?
python3 scripts/drift-check.py --instance <inst> --show-diff    # wenn nicht
```
...
```bash
python3 scripts/capture.py --instance <inst>
python3 scripts/normalize.py --write apps/<app>/flows.json
...
- [ ] Der Fleet-Lauf ist grün:
      `python3 scripts/drift-check.py --all --json inventory/drift.json` — keine
